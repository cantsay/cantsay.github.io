...
$("#calculate").click(function () {
  ...
  var damageRollSets = createArray(numSets, 16);

  for (var i = 0; i < numSets; i++) {
    damageRollSets[i] = parseDamageRolls($(sets[i]).val());
  }
  ...
  for (var a = 0; a < numSets; a++) {
    if ($(sets[a]).val().length > 0) rollSize += 1;
  }
...
$("#removeRoll").click(function () {
  var container = document.getElementById("roll-container");
...
function addSet() {
	...
	e.textContent = "Set " + (count + 1) + ": ";
	var c = document.createElement("input");
	c.classList.add("setInput");
...
function getKOChance2(HPStat, combinedRolls, beginIndex, dmgRolls) {
  ...
  if (beginIndex >= dmgRolls.length) {
    ...
    $("#percentOut").val(
      (100.0 * count) / combinedRollsSize + "% chance to KO"
    );
